...
def _resolve_fastlio_config_file(config_path: str, config_file: str) -> Path:
    config_file_text = str(config_file).strip()
    if not config_file_text:
        config_file_text = "mid360.yaml"
    config_path_text = str(config_path).strip()
    if not config_path_text:
        config_path_text = str(Path(__file__).resolve().parents[2] / "config")

    cfg_path = Path(config_file_text)
    if cfg_path.is_absolute():
        return cfg_path

    return Path(config_path_text) / cfg_path


def _load_fastlio_camera_offsets(config_path: str, config_file: str) -> Tuple[np.ndarray, np.ndarray, str]:
    cfg_path = _resolve_fastlio_config_file(config_path, config_file)
    with cfg_path.open("r", encoding="utf-8") as handle:
        data = yaml.safe_load(handle) or {}

    ros_params = data.get("/**", {}).get("ros__parameters", {})
    mapping = ros_params.get("mapping", {})
    t_values = mapping.get("camera_offset_T")
    r_values = mapping.get("camera_offset_R")
    if not isinstance(t_values, (list, tuple)) or len(t_values) != 3:
        raise ValueError("mapping.camera_offset_T must contain 3 values")
    if not isinstance(r_values, (list, tuple)) or len(r_values) != 9:
        raise ValueError("mapping.camera_offset_R must contain 9 values")
...
class CornerLidarSolverNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("fastlio_config_path", "")
        self.declare_parameter("fastlio_config_file", "")
        ...
        fastlio_config_path = self.get_parameter("fastlio_config_path").get_parameter_value().string_value
        fastlio_config_file = self.get_parameter("fastlio_config_file").get_parameter_value().string_value
        try:
            self.t_cb, self.R_cb, cfg_source = _load_fastlio_camera_offsets(
                fastlio_config_path,
                fastlio_config_file,
            )
            self.get_logger().info(f"Loaded solver camera extrinsics from Fast-LIO config: {cfg_source}")
        except Exception as exc:
            raise RuntimeError(f"Failed to load solver camera extrinsics from Fast-LIO config: {exc}") from exc
