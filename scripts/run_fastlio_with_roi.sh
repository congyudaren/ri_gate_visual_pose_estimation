...
DEFAULT_LAUNCH_ARGS=(
  ...
  "min_points:=${ROI_LIDAR_CORNER_MIN_POINTS:-1}"
  "max_time_diff_cloud:=${ROI_LIDAR_CORNER_MAX_TIME_DIFF_CLOUD:-0.25}"
  "max_time_diff_odom:=${ROI_LIDAR_CORNER_MAX_TIME_DIFF_ODOM:-0.08}"
  "min_range:=${ROI_LIDAR_CORNER_MIN_RANGE:-0.2}"
  "max_range:=${ROI_LIDAR_CORNER_MAX_RANGE:-30.0}"
  "history_window_sec:=${ROI_LIDAR_CORNER_HISTORY_WINDOW_SEC:-0.8}"
  "max_window_frames:=${ROI_LIDAR_CORNER_MAX_WINDOW_FRAMES:-8}"
  "cache_voxel_size:=${ROI_LIDAR_CORNER_CACHE_VOXEL_SIZE:-0.1}"
  "bbox_expand_ratio:=${ROI_LIDAR_CORNER_BBOX_EXPAND_RATIO:-0.15}"
  "corner_target_points:=${ROI_LIDAR_CORNER_CORNER_TARGET_POINTS:-24}"
  ...
  "open_debug_window:=${ROI_LIDAR_CORNER_OPEN_DEBUG_WINDOW:-true}"
  "rviz:=${ROI_LIDAR_CORNER_ENABLE_RVIZ:-true}"
